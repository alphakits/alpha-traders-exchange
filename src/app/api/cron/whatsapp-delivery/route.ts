...
import { runWhatsAppDeliverySweep } from "@/lib/whatsapp-notifications";
...
export async function GET(request: NextRequest) {
  ...
  if (secret.length < 32) {
    logEvent("error", {
      event: "whatsapp_delivery_cron",
      outcome: "failed",
      reason: "cron_secret_not_configured",
    });
    return NextResponse.json(
      { error: "WhatsApp delivery scheduler is not configured." },
  ...
  try {
    const result = await runWhatsAppDeliverySweep();
    logEvent("info", {
      event: "whatsapp_delivery_cron",
      outcome: "success",
      metadata: result,
    ...
    return NextResponse.json(
      { error: "WhatsApp delivery sweep failed." },
