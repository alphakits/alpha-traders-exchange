...
describe("API mutation origin protection", () => {
  ...
  it("allows same-origin authenticated write requests to continue", async () => {
  ...
  it("fails closed when a protected API mutation has no Origin header", async () => {
    const response = (await middleware(apiRequest(
  ...
  it("allows originless native mobile mutations only with required client headers", async () => {
    const response = (await middleware(apiRequest(
  ...
  it("rejects native-looking cross-site browser mutations", async () => {
    const response = (await middleware(apiRequest(
      "/api/mobile/v1/auth/login",
      "POST",
      {
  ...
  it("preserves loopback-only production E2E writes without weakening deployed production", async () => {
    ...
    const localResponse = (await middleware(new NextRequest(
    ...
    const localCrossSiteResponse = (await middleware(new NextRequest(
      "http://127.0.0.1:3000/api/testing/alpha-exchange-state",
      {
        method: "PUT",
        headers: {
    ...
    const deployedResponse = (await middleware(new NextRequest(
  ...
  it.each([
    ...
    "/api/twilio/status",
  ])("preserves authenticated external callback delivery for %s", async (pathname) => {
    const response = (await middleware(apiRequest(pathname, "POST")));

    expect(response.status).toBe(200);
    expect(response.headers.get("x-middleware-next")).toBe("1");
  });

  it("does not block read-only API requests without an Origin header", async () => {
    const response = (await middleware(apiRequest("/api/health", "GET")));

    expect(response.status).toBe(200);
    expect(response.headers.get("x-middleware-next")).toBe("1");
  });

  it("allows an email-verified seller into the workspace without a phone cookie in email-only mode", async () => {
