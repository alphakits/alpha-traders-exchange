...
it("keeps legacy approved status for reconciliation without granting the seller role", async () => { const u = { ...legacy("legacy-approved"), role: "approved_seller", roles: ["buyer", "approved_seller"], sellerStatus: "approved_seller" }; loadSnapshot.mockResolvedValue(base(u)); const r = await findUserById("legacy-approved"); expect(hasRole(r!, "approved_seller")).toBe(false); expect(r?.role).toBe("buyer"); expect(r?.sellerStatus).toBe("approved_seller"); });
