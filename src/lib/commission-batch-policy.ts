...
/** Must be obtained from a persisted, authenticated owner approval, NOT request-body assertions. */
...
export type BatchRejection =
  ...
  | "amount_changed" | "outside_tolerance" | "before_commission";
...
/** Apply tolerance ONCE to the entire authorized group, never per invoice. */
export function planCommissionBatch(input: {
  records: readonly BatchCommission[];
  receipt: BatchReceipt;
  attribution: BatchReceiptAttribution | null;
  now: number;
}): CommissionBatchPlan {
  ...
  // Every recorded reference remains reserved, including manual/failed records.
  // Releasing a reference requires an explicit audited investigation, not a fuzzy rematch.
  ...
  for (const id of [...ids].sort()) {
    ...
    if (!Number.isFinite(created) || receipt.timestamp < created - COMMISSION_BATCH_CLOCK_SKEW_MS) {
      return reject("before_commission");
    }
  ...
  let allocated = 0;
  const allocations = group.map(({ record, micros }, index) => {
    // Integer, deterministic proportional allocation; the last row receives only
    // the fractional-micro rounding remainder, not a second tolerance allowance.
    const receivedMicros = index === group.length - 1 ? receipt.amountMicros - allocated
      : Number(BigInt(receipt.amountMicros) * BigInt(micros) / expected);
    allocated += receivedMicros;
    return { commissionId: record.id, amountDueMicros: micros, receivedMicros,
      waivedMicros: Math.max(0, micros - receivedMicros), excessMicros: Math.max(0, receivedMicros - micros) };
  });
