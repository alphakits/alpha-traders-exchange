...
export async function provisionQaAccount(input: {
  email: string;
  fullName: string;
  whatsappNumber: string;
  passwordHash: string;
  ...
  const baseUser = await upsertUserProfileForAuth({
  ...
  const db = await readDb();
  const index = db.users.findIndex((user) => user.id === baseUser.id);
  if (index === -1) throw new Error("User not found after provisioning.");
  ...
  db.users[index] = {
    ...
    verifiedPhone: input.verifiedPhone ?? existing.verifiedPhone,
    phoneVerifiedAt: input.verifiedPhone ? (existing.phoneVerifiedAt ?? now) : existing.phoneVerifiedAt,
    buyerVerificationStatus: input.verifiedPhone ? "verified" : existing.buyerVerificationStatus,
    onboardingSelection: input.onboardingSelection ?? existing.onboardingSelection,
    onboardingCompletedAt: input.onboardingSelection ? (existing.onboardingCompletedAt ?? now) : existing.onboardingCompletedAt,
    preferredNetworks:
      input.sellerStatus === "approved_seller"
        ? (existing.preferredNetworks.length ? existing.preferredNetworks : ["TRC20"])
        : existing.preferredNetworks,
    isProfileHidden: false,
    showTradeStats: true,
    showLastActive: true,
    allowDirectMessages: true,
...
export async function disableQaAccountsByEmails(emails: string[]) {
  const normalizedEmails = new Set(emails.map((value) => normalizeEmail(value)).filter(Boolean));
  if (!normalizedEmails.size) return { disabled: 0 };
  const db = await readDb();
  let disabled = 0;
  const now = nowIso();
  ...
  for (let i = 0; i < db.users.length; i += 1) {
    ...
    if (!normalizedEmails.has(normalizeEmail(user.email))) continue;
    disabled += 1;
    const disabledRoles = normalizeRolesForUser({
      email: user.email,
      roles: ["guest"],
    ...
    db.users[i] = {
      ...user,
      roles: disabledRoles,
      role: resolvePrimaryRole(disabledRoles),
      sellerStatus: "suspended",
