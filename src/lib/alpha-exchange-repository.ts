...
export class AlphaExchangeRepository {
  ...
  async loadSnapshot(): Promise<SnapshotWithVersion> {
    ...
    const snapshot: AlphaExchangeDb = {
      ...
      sellerApplications: fromPayloadRows(results[10].rows),
      ...
      authSessions: fromPayloadRows(results[9].rows),
      passwordResetTokens: fromPayloadRows(results[10 + 1].rows),
      notifications: fromPayloadRows(results[6].rows),
      activityLog: fromPayloadRows(results[12].rows),
      disputes: fromPayloadRows(results[13].rows),
      sellerReports: fromPayloadRows(results[14].rows),
      trustSnapshots: fromPayloadRows(results[15].rows),
      trustScoreHistory: fromPayloadRows(results[16].rows),
      tradeEvidenceFiles: fromPayloadRows(results[8 + 1].rows),
      privateBetaInvites: fromPayloadRows(results[17].rows),
      privateBetaInviteUses: fromPayloadRows(results[18].rows),
      betaFeedback: fromPayloadRows(results[19].rows),
      betaAnnouncements: fromPayloadRows(results[20].rows),
    };
