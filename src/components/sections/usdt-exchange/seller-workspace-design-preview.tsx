...
import { BellRing, HandCoins, ShieldCheck, Store, TrendingUp, Trophy, Upload } from "lucide-react";
...
export function SellerWorkspaceDesignPreview({ locale }: { locale: "en" | "ar" }) {
  ...
  const cards: ExchangeWorkspaceAction[] = [
    { key: "create-listing", title: isAr ? "إنشاء عرض" : "Create Listing", subtitle: isAr ? "نشر عرض جديد" : "Publish a new listing", stat: isAr ? "إنشاء عرض" : "Create listing", icon: Upload, tone: "gold", onClick: previewAction },
  ...
  return (
    <main className="section-container py-6" dir={isAr ? "rtl" : "ltr"}>
      ...
      <AccountWelcome
        ...
        workspace={isDesktopWorkspace ? <ExchangeWorkspaceNavigation cards={cards} isAr={isAr} integrated /> : undefined}
      >
      ...
      {!isDesktopWorkspace ? <ExchangeWorkspaceNavigation cards={cards} isAr={isAr} /> : null}
