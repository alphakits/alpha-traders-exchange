...
export function SellerApplicationSection({
  ...
  return (
    <div className={`${prominent ? "mt-5" : "mt-10"} grid ${compact ? "gap-4" : "gap-6"}`}>
        ...
        <CardContent className="space-y-4">
          {shouldCondense && !isExpanded ? (compact ? (
            ...
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-[#D1D5DB]">
              <p className="text-xs uppercase tracking-[0.14em] text-[#D4AF37]">{currencyText(prominent ? (isAr ? "بع USDT على Alpha Exchange" : "Sell on Alpha Exchange") : (isAr ? "خيار إضافي" : "Optional Next Step"))}</p>
              <p className="mt-2 text-base font-semibold text-white">{currencyText(isAr ? "هل تريد البيع أيضًا؟" : "Want to sell USDT too?")}</p>
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-[#C9A227]/35 bg-[#C9A227]/10 p-5"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-full border border-[#C9A227]/50 bg-[#C9A227]/20"><Clock3 className="h-5 w-5 text-[#F4D87A]" /></div><div><p className="font-semibold text-white">{isAr ? "الطلب قيد المراجعة" : "Application Pending Review"}</p><p className="mt-0.5 text-xs text-[#D1D5DB]">{currencyText(application?.createdAt ? `${isAr ? "تاريخ التقديم" : "Submitted"}: ${new Date(application.createdAt).toLocaleDateString(isAr ? "ar-IL" : "en-IL")}` : (isAr ? "تم إرسال الطلب" : "Application submitted"))}</p></div></div></div>
            ...
            <>
              ...
              {statusMessage ? <ActionFeedback revealKey={statusMessageFeedbackKey} className="flex items-center gap-2 rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-[#FDE68A]"><AlertTriangle className="h-3.5 w-3.5 shrink-0" /><span>{currencyText(statusMessage)}</span></ActionFeedback> : null}
              <form className="space-y-3" onSubmit={onSubmit}>
                ...
                <div className="space-y-1.5"><FieldLabel htmlFor="seller-whatsapp" required>{isAr ? "رقم WhatsApp للتواصل" : "WhatsApp Contact Number"}</FieldLabel><Input id="seller-whatsapp" placeholder={isAr ? "رقم WhatsApp للتواصل" : "WhatsApp contact number"} value={form.whatsappNumber} required aria-required onChange={(event) => onFormChange("whatsappNumber", event.target.value)} className={requiredFieldClasses({ value: form.whatsappNumber, required: true })} /><p className="text-xs text-sky-200">{isAr ? "للتواصل بخصوص الطلب فقط؛ لن نرسل رمز تحقق إلى هذا الرقم." : "For application contact only; no verification code will be sent to this number."}</p></div>
