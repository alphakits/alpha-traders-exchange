    ...
    <div className={`${prominent ? "mt-5" : "mt-10"} grid gap-6 xl:grid-cols-2`}>
      <Card id="seller-application" className={prominent ? "border-[#C9A227]/45 bg-[linear-gradient(145deg,rgba(201,162,39,0.13),rgba(11,11,11,0.96)_48%)] shadow-[0_18px_65px_rgba(201,162,39,0.12)]" : "border-white/10 bg-[#0B0B0B]/90"}>
        ...
        <CardContent className="space-y-4">
          {shouldCondense && !isExpanded ? (
          ...
          ) : eligibility === "verification_reconciliation_required" ? (
            <div className="rounded-2xl border border-amber-500/35 bg-amber-500/10 p-5">
              <div className="flex items-start gap-3">
                <AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-amber-300" />
                <div>
                  <p className="font-semibold text-white">{isAr ? "يلزم مطابقة سجل التحقق" : "Verification Record Reconciliation Required"}</p>
                  <p className="mt-2 text-sm text-[#E5E7EB]">
                    {isAr
                      ? "حسابك معتمد، لكن أدوات نشر العروض وبدء الصفقات الجديدة ستبقى مقفلة حتى يسجّل مسؤول مخوّل مراجعة الهوية الحكومية وفيديو الهوية وملكية وسيلة التواصل والموافقة على قواعد السوق."
                      : "Your account is approved, but listing and new-trade tools remain locked until an authorized reviewer records the completed government-ID, live-video, contact-ownership, and marketplace-rules checks."}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-emerald-500/35 bg-emerald-500/10 p-5">
              ...
              <p className="mt-2 text-sm text-[#E5E7EB]">
                {isAr
                  ? "اكتملت مطابقة سجل التحقق. استخدم لوحة البائع لإدارة العروض والصفقات."
                  : "Your verification record is complete. Use the seller dashboard to manage listings and trades."}
              </p>
            </div>
